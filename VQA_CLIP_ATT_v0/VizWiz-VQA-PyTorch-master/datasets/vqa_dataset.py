...
class VQADataset(data.Dataset):
    ...
    def __init__(self, config, split):
        ...
        with h5py.File(config['images']['path_features'], 'r') as f:
            img_names = f['image'][()]
        # Create mapping with both bytes and string keys for compatibility
        self.name_to_id = {}
        for i, name in enumerate(img_names):
            if isinstance(name, bytes):
                # Add both bytes and decoded string as keys
                self.name_to_id[name] = i
                self.name_to_id[name.decode('utf-8')] = i
            else:
                # Add both string and encoded bytes as keys
                self.name_to_id[name] = i
                self.name_to_id[name.encode('utf-8')] = i
    ...
    def __getitem__(self, i):
        ...
        # Get feature ID - try string first, then bytes
        feature_id = None
        if img_name in self.name_to_id:
            feature_id = self.name_to_id[img_name]
        elif isinstance(img_name, str) and img_name.encode('utf-8') in self.name_to_id:
            feature_id = self.name_to_id[img_name.encode('utf-8')]
        elif isinstance(img_name, bytes) and img_name.decode('utf-8') in self.name_to_id:
            feature_id = self.name_to_id[img_name.decode('utf-8')]
        else:
            raise KeyError(f"Image name '{img_name}' not found in feature mapping")
